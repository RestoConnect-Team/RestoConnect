...
import { fetchCenterDetail, CenterDetail, ContactInfo, CenterAlert } from "@/lib/api/center_detail_info";
...
export default function CenterDetailPage({
  ...
  const { id } = use(params);
  const { data: center, loading, error } = useFetchData<CenterDetail>(
        ...
        <div className="flex gap-4 text-[13px]">
          ...
          <div className="space-y-1 text-gray-800">
            {scheduleEntries.map(([day, slots]) =>
              slots.map((slot, i) => (
                <p key={`${day}-${i}`}>
                  {day}, {formatTime(slot.opening_time)} à {formatTime(slot.closing_time)}
                </p>
              ))
            )}
          </div>
      ...
      {center.alerts.length > 0 && (
        <SectionCard>
          <div className="flex items-center justify-between mb-1">
            <h2 className="text-[15px] font-bold text-gray-900">Dernières alertes</h2>
