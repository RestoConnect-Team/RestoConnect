...
export default function ScanQrPage() {
  ...
  const handleScanResult = async (reference: string) => {
    ...
    try {
      ...
      if (data.status === StockStatus.LOST) {
        await updateStatus(data.id, StockStatus.AVAILABLE);
        setProduct({ ...data, status: StockStatus.AVAILABLE });
      } else {
        setProduct(data);
      }
    } catch (err: any) {
  ...
  const updateStatus = async (
    ...
    try {
      const response = await fetch(
        `http://localhost:8000/api/stock/${productId}/status`,
        {
          method: "PATCH",
          credentials: "include",
          headers: {
  ...
  return (
    <div className="fixed inset-0 bg-gray-900 text-white flex flex-col z-50">
      ...
      <div className="relative flex-1 overflow-hidden">
        ...
        <div
          className={`absolute inset-0 flex flex-col items-center justify-center transition-all duration-300 ease-in-out ${product ? "pb-40" : "pb-0"}`}
        >
          <ScannerOverlay />
